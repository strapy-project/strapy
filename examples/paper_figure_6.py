###
#   This script produces Figure 6 in "Polarisation-sensitive transfer matrix
...
for i, x in enumerate(xs):
    ...
    ints1analytic[i] = np.abs(0.5*fOutS + 0.5*fOutP*np.exp(1j*np.pi/2))**2
    ints2analytic[i] = np.abs(0.5*fOutS + 0.5*fOutP)**2
...
ax0.plot(xs, ints1analytic, label='PD1 (analytic)', color=red, ls=':')
ax0.plot(xs, ints2analytic, label='PD2 (analytic)', color=red, ls='-.')
